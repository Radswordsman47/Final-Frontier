...
public abstract partial class SharedProjectileSystem : EntitySystem
{
    ...
    public virtual DamageSpecifier? ProjectileCollide(Entity<ProjectileComponent, PhysicsComponent> projectile, EntityUid target, MapCoordinates? collisionCoordinates, bool predicted = false)
    {
		...
		var user = EntityManager.EntityExists(component.Shooter) ? component.Shooter!.Value : component.Weapon!.Value;
        if (projectile.Comp1.DamagedEntity && !component.AlwaysReflect)
        {
		...
		if (component.AlwaysReflect)
		{
			...
			var directionHere = ourBody.LinearVelocity.Normalized();
			var speed = Vector2.Distance(Vector2.Zero, ourBody.LinearVelocity);
			var normal = new Vector2(coordsBetween.X,coordsBetween.Y).Normalized();
			var xHere = directionHere.X;
			var yHere = directionHere.Y;
			var otherCalc = normal * (directionHere * normal);
			var calculated = new Vector2(xHere - 2 * otherCalc.X, yHere - 2 * otherCalc.Y).Normalized();
			//var gotAngle = (Angle)(Math.Atan2(directionHere.Y, directionHere.X) * (180/3.14));
			//var newVelocity = gotAngle.RotateVec(ourBody.LinearVelocity);
			_physics.SetLinearVelocity(projectile, calculated * 45, body: ourBody);
		}
		...
		if (component.AlwaysReflect)
		{
			//component.AlwaysReflect = false;
			component.DeleteOnCollide = true;
			component.Shooter = user;
            component.Weapon = user;
            Dirty(uid, component);
		}
