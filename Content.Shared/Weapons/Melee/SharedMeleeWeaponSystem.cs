...
public abstract class SharedMeleeWeaponSystem : EntitySystem
{
    ...
    private bool AttemptAttack(EntityUid user, EntityUid weaponUid, MeleeWeaponComponent weapon, AttackEvent attack, ICommonSession? session)
    {
        ...
        if (EntityManager.TryGetComponent(weaponUid, out GunComponent? gunComponent))
        {
            if (EntityManager.TryGetComponent(weaponUid, out ItemSlotsComponent? slotComponent))
            {
                bool hasBayonet = false;
                foreach (var e in slotComponent.Slots)
                {
                    if (e.Value.ID == "bayonet" || e.Value.Name == "Bayonet")
                    {
                        hasBayonet = true;
                        if (e.Value.HasItem)
                        {
                            if (e.Value.Item != null)
                            {
                                if (EntityManager.TryGetComponent(e.Value.Item, out MeleeWeaponComponent? bayonetMelee))
                                {
                                    weapon = bayonetMelee;
                                }
                            }
