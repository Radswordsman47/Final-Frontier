    ...
    public sealed class PoweredLightSystem : EntitySystem
    {
		...
		public override void Update(float frameTime)
        {
			if (RobustRandom.Prob(0.25f))
			{
				...
				{
					if (EntityManager.TryGetComponent(uid, out PointLightComponent? pointLight) && EntityManager.TryGetComponent(uid, out PoweredLightComponent? poweredLight))
					{
						var bulbUid = GetBulb(uid, poweredLight);
						if (bulbUid != null && EntityManager.TryGetComponent(bulbUid.Value, out LightBulbComponent? lightBulb))
						{
							if (lightBulb.LightRadius != null)
								_pointLight.SetRadius(uid, (float) lightBulb.LightRadius * randRad, pointLight);
							if (lightBulb.LightEnergy != null)
								_pointLight.SetEnergy(uid, (float) lightBulb.LightEnergy * randNrg, pointLight);
						}
					}
				}
			}
        ...
        private void SetLight(EntityUid uid, bool value, Color? color = null, PoweredLightComponent? light = null, float? radius = null, float? energy = null, float? softness = null)
        {
            ...
            if (EntityManager.TryGetComponent(uid, out PointLightComponent? pointLight))
            {
                ...
                if (radius != null)
                    _pointLight.SetRadius(uid, (float) radius * RobustRandom.NextFloat(0.75f,1.25f), pointLight);
                if (energy != null)
                    _pointLight.SetEnergy(uid, (float) energy * RobustRandom.NextFloat(0.75f,1.25f), pointLight);
                if (softness != null)
