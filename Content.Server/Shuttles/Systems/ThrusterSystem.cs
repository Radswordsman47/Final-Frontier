...
using Content.Shared.Atmos.Prototypes; // Frontier
...
public sealed class ThrusterSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ThrusterComponent, AtmosDeviceUpdateEvent>(OnHydrogenThrusterUpdate);
    }
    ...
    /// <summary>
    /// FINAL FRONTIER: Update logic for hydrogen thrusters.
    /// </summary>
    private void OnHydrogenThrusterUpdate(EntityUid uid, ThrusterComponent component, ref AtmosDeviceUpdateEvent args)
    {
        if (!component.IsOn)
            return;

        if (!component.IsHydrogen)
            return;

        if (component.IsHydrogen && _nodeContainer.TryGetNode(uid, "pipe", out PipeNode? inlet))
        {
            var removed = inlet.Air.Remove(0.25f);
            var hasFuel = false;
            ...
            if (!hasFuel && component.IsOn)
                DisableThruster(uid, component);
            if (hasFuel && CanEnable(uid, component))
                EnableThruster(uid, component);
        }
    ...
    public bool CanEnable(EntityUid uid, ThrusterComponent component)
    {
        ...
        if (component.IsHydrogen && _nodeContainer.TryGetNode(uid, "pipe", out PipeNode? inlet))
        {
            var removed = inlet.Air.Remove(0.1f);
            var hasFuel = false;
            for (var i = 0; i < Atmospherics.TotalNumberOfGases; i++)
            {
                var moles = removed[i];
