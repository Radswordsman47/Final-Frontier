...
public sealed class PlanetSystem : EntitySystem
{
    ...
    public EntityUid? LoadPlanet(ProtoId<PlanetPrototype> id, ResPath path)
    {
        ...
        EntityUid gridUid;
        gridUid = grid.Value;
        List<EntityUid> gridList = [grid.Value];
        ...
        {
            if (location.SpawnGroup == "Planetary")
            {

                proto = location;
                string stationName = proto.Name;
                ...
                var meta = EnsureComp<MetaDataComponent>(grid.Value);
                //_meta.SetEntityName(grid.Value, stationName, meta);

                EntityManager.AddComponents(grid.Value, proto.AddComponents);
